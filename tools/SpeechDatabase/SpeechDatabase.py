...
speechFile = 'ohbotData/OhbotSpeech.csv'
# directory = ohbot.dir
numberOfRows = 0
phraseList = []
rowList = []
    ...
    for row in data:
        ...
        if row[0] != '' and row[0] != 'Set':
            if row[0] == '' and row[1] == '':
                ...
                phraseList.append(newPhrase)
root = Tk.Tk()
root.title("Ohbot - Speech Database")
...
imageFile = os.path.join(directory, 'Images/ohbotlogoSmall.gif')
ohbotImage = Tk.PhotoImage(file=imageFile)
ohbotLabel = Tk.Label(frameTwo, image=ohbotImage)
ohbotLabel.grid(column=2, row=1, sticky='w')
