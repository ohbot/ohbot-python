...
def Database(name, set, variable):
    return ohbot.getPhrase(set, variable)

# Map the SayDB block to getPhrase in the library.  Name is unused
def SayDatabase(name, set, variable):
    phrase = ohbot.getPhrase(set, variable)
    ohbot.say(phrase, lipSync=False)
...
def WhenFlagClickedThread0 ():
    ...
    while (running):
        ...
        ohbot.wait(0.01)
        ohbot.move (ohbot.HEADTURN,ht)
        # add a slight headnod when speaking to show emphasis
        ohbot.move (ohbot.HEADNOD,hn-(ohbot.getTopLip()-5)/5)
        ohbot.move (ohbot.EYETURN,15-ht*2)
        ohbot.move (ohbot.EYETILT,15-hn*2)
...
def WhenFlagClickedThread4 ():
    ...
    while (running):
        # prevent thread block
        ohbot.wait(0.1)
        ohbot.move (ohbot.TOPLIP,ohbot.getTopLip ())
        ohbot.move (ohbot.BOTTOMLIP,ohbot.getBottomLip ())
...
def WhenKeyPressedspaceThread0 ():
    ...
    showInfo(content)
    ohbot.say (content,lipSync=False)
...
print ("Using Ohbot library version:" + ohbot.version)
